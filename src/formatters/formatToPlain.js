  ...
  .flatMap((node) => {
    switch (node.type) {
      case 'added': {
        ...
        return `Property '${getPath(parentNames, key)}' was added with value: ${stringify(value)}`;
      }
      case 'deleted': {
        const { key } = node;
      ...
      case 'changed': {
        ...
        return `Property '${getPath(parentNames, key)}' was updated. From ${stringify(value1)} to ${stringify(value2)}`;
      }
      case 'nested': {
